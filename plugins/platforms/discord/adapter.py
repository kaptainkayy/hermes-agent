...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._secondbrain_pending: Dict[tuple[str, str], Dict[str, Any]] = {}
    ...
    def _set_secondbrain_pending(self, *, user_id: str, channel_id: str, corpus_key: str, label: str) -> None:
        self._secondbrain_pending[self._secondbrain_context_key(user_id, channel_id)] = {
    ...
    def _load_secondbrain_index_for_discord(self):
        ...
        commands = get_plugin_commands()
        if "secondbrain" not in commands:
            raise RuntimeError("secondbrain plugin command is not registered")
        ...
        try:
            return module.load_or_refresh_corpus_index(connection)
        finally:
            connection.close()

    async def _handle_secondbrain_picker_slash(self, interaction: "discord.Interaction") -> None:
        if not await self._check_slash_authorization(interaction, "/secondbrain"):
            return
        try:
            index = self._load_secondbrain_index_for_discord()
        except Exception:
            logger.warning("Failed to load Second Brain corpus index for Discord picker.")
            await interaction.response.send_message(
                "I can’t load the Second Brain index. Try `/secondbrain refresh`.",
                ephemeral=True,
            )
            return
    ...
    def _register_slash_commands(self) -> None:
        ...
        async def slash_secondbrain(interaction: discord.Interaction):
            await self._handle_secondbrain_picker_slash(interaction)
...
def _define_discord_view_classes() -> None:
    ...
    class SecondBrainCorpusSelectView(discord.ui.View):
        """Select a Second Brain corpus from a loaded index."""

        def __init__(self, adapter, index, allowed_user_ids: set, allowed_role_ids: Optional[set] = None):
            ...
            options = []
            for entry in index.enabled_entries()[:25]:
                rows = f"{entry.row_count:,} rows"
                options.append(
                    discord.SelectOption(
            ...
            select = discord.ui.Select(
                placeholder="Choose a Second Brain database...",
                options=options,
                custom_id="secondbrain_corpus_select",
            )
            select.callback = self._on_corpus_selected
            self.add_item(select)
        ...
        async def _on_corpus_selected(self, interaction: "discord.Interaction"):
            if self.resolved:
                await interaction.response.send_message("This Second Brain picker is already resolved.", ephemeral=True)
                return
            if not self._check_auth(interaction):
                await interaction.response.send_message("You're not authorized~", ephemeral=True)
                return

            corpus_key = interaction.data["values"][0]
            entry = self.index.get_entry(corpus_key)
            self.adapter._set_secondbrain_pending(
                user_id=str(interaction.user.id),
                channel_id=str(interaction.channel_id),
